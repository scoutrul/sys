...
class Envato_WP_Toolkit {
  ...
  /*
  * Check theme for update
  * Author: 8theme
  */
  public function checkForUpdate($theme_name,$showMsg = false) {    
	/* read in existing API value from database */
	$options = get_option( EWPT_PLUGIN_SLUG );
	...
	/* get WP current themes */
	$currentTheme = wp_get_theme($theme_name);
	...
	if($has_update) {
	 if($showMsg){
			$update_url = wp_nonce_url( network_admin_url( 'admin.php?page=' . EWPT_PLUGIN_SLUG . '&action=upgrade-theme&amp;theme=' . $current_stylesheet . '&amp;item_id=' . $item_id ), 'upgrade-theme_' . $current_stylesheet );
			$update_onclick = 'onclick="if ( confirm(\'' . esc_js( __( "Updating this theme will lose any customizations you have made. 'Cancel' to stop, 'OK' to update.", 'envato' ) ) . '\') ) {return true;}return false;"';
			
			echo '<div id="setting-error-tgmpa" class="updated settings-error"> ';
			echo '<p><strong>';
			echo __('There is a new version of ', ETHEME_DOMAIN).$title.__(' available.', ETHEME_DOMAIN);
			echo ' <a href="'.$update_url.'"' . $update_onclick .'>';
			echo __('Update me!', ETHEME_DOMAIN);
			echo '</a> ';
			echo '<br/> ';
			echo __('(<em>Only for WooCoommerce 2.0.X+</em>)', ETHEME_DOMAIN);
			echo '</strong></p>';
			echo '</div>';
		}else{
			return TRUE;
